...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({
    ...
    const labelSettingsSummary = `${
        labelShape === "round" ? `${t("tonies.teddystudio.round")} ${diameter}` : `${width} x ${height}`
    }`;

    const printModeSummary = t(`tonies.teddystudio.printMode${printMode.charAt(0).toUpperCase() + printMode.slice(1)}`);

    const paperSizeSummary =
        paperSize === "Custom" ? `${paperSize} (${customPaperWidth} x ${customPaperHeight})` : paperSize;
