    ...
    return (
        <div style={{ flex: `0 0 calc(${100 / gridColumns}% - 16px)`, maxWidth: `calc(${100 / gridColumns}% - 16px)` }}>
            <Card hoverable={false} size="small" style={{ background: token.colorBgContainerDisabled, height: "100%", display: "flex", flexDirection: "column" }}
                title={<div style={{ overflow: "hidden", textOverflow: "ellipsis" }}>{entry.series || entry.model || "-"}</div>}
                cover={<div style={{ position: "relative" }} onClick={() => { if (previewUrl) onPreviewClick(previewUrl); }}>
                    {previewUrl ? <img src={toImageSrc(previewUrl)} alt={entry.series || entry.model || ""} style={{ padding: 8, width: "100%" }} /> : <img src={toImageSrc("/img_unknown.png")} alt="" style={{ padding: 8, paddingTop: 10, width: "100%" }} />}
                </div>}
                actions={[
                    <Tooltip key="edit" title={t("tonies.customEditor.actions.edit")}><span onClick={() => onEdit(idx)} style={{ cursor: "pointer" }} onKeyDown={(e) => e.key === "Enter" && onEdit(idx)} role="button" tabIndex={0}><EditOutlined /></span></Tooltip>,
                    <Tooltip key="dup" title={t("tonies.customEditor.actions.duplicate")}><span onClick={() => onDuplicate(idx)} style={{ cursor: "pointer" }} onKeyDown={(e) => e.key === "Enter" && onDuplicate(idx)} role="button" tabIndex={0}><CopyOutlined /></span></Tooltip>,
                    <Popconfirm key="del" title={t("tonies.customEditor.deleteConfirm.title")} description={t("tonies.customEditor.deleteConfirm.description", { model: entry.model })} onConfirm={() => void onDelete(idx)} okText={t("tonies.customEditor.deleteConfirm.confirm")} cancelText={t("tonies.customEditor.deleteConfirm.abort")}>
                        <Tooltip title={t("tonies.customEditor.actions.delete")}><span style={{ cursor: "pointer", color: token.colorError }} role="button" tabIndex={0}><DeleteOutlined /></span></Tooltip>
                    </Popconfirm>,
                ]}
            >
                {(() => {
                    ...
                    const metaDescription = entry.model && entry.model.trim() !== displayName ? entry.model : undefined;
