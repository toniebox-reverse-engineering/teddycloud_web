  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const filteredData = jsonData.filter((item: any) => item.audio_id && item.audio_id.length === 0 && !["creative-tonie", "system"].includes(item.category) && !item.model.includes("20000"));
    ...
    <>
      ...
      <StyledLayout>
        ...
        <StyledBreadcrumb items={[
          ...
          { title: t("community.contribution.navigationTitle") }, { title: t("community.contribution.toniesJson.navigationTitle") }
        ]} />
        <StyledContent>
          <h1>{t(`community.contribution.toniesJson.title`)}</h1>
          <Paragraph>
            {t("community.contribution.toniesJson.text")}
          </Paragraph>
          <Paragraph>
            <Collapse accordion>
